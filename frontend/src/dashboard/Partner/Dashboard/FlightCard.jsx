...
import { Bus, MoveRight, Plane, Route } from 'lucide-react';
...
const FlightCard = () => {
    ...
    useEffect(() => {
        const dataFetch = async () => {
            try {
                const res = await AxiosToken.get("/dashboard/partner/my-flight");
        ...
        <Box bg="white" borderRadius="2xl" border="1px solid" borderColor="gray.100"
            ...
            <Flex px={5} py={4} borderBottom="1px solid" borderColor="gray.100"
                ...
                <Flex align="center" gap={2}>
                    ...
                    <Text fontSize="sm" fontWeight={700} color="gray.800">Mes vols</Text>
                </Flex>
            </Flex>
            <Box overflowX="auto">
                <Box as="table" w="full" style={{ borderCollapse: "collapse" }}>
                    <Box as="thead">
                        <Box as="tr" bg="gray.50">
                            {["numéro de Vol", "Type de vol","vol", "Réservations", "Occupation"].map(h => (
                    ...
                    <Box as="tbody">
                        {
                            flights && flights.length === 0 ?
                                ...
                                flights.map((flight, i) => {
                                    ...
                                    return (
                                        <Box key={flight.flight_id} as="tr"
                                                ...
                                                <Flex align="center" gap={2}>
                                                    <Box flex={1} bg="gray.100" borderRadius="full" h="5px" overflow="hidden" maxW="80px">
                                                        <Box h="100%" borderRadius="full"
                                                            bg={flight.status ? "#7C3AED" : "#CBD5E1"}
                                                            style={{ width: `${flight.occupation}%`, transition: "width 0.4s" }} />
                                                    </Box>
                                                    <Text style={{ fontSize: "10px", fontWeight: 700, color: "#64748B" }}>{flight.occupation}%</Text>
                                                </Flex>
                                            </Box>
                                        </Box>
                                    )
