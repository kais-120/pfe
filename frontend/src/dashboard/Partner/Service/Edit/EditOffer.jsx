...
import {
  ...
  LuCheck, LuX, LuGlobe, LuClock, LuAlignLeft,
  LuPackage, LuListTree, LuPlus, LuArrowRight,
  LuSearch, LuBanknote, LuPencil, LuTrash2,
} from "react-icons/lu"
...
const createValidationSchema = (hasExistingImages = false) => {
  return Yup.object({
    ...
    images: Yup.array()
      .test('has-images', 'Veuillez ajouter au moins une image', function(value) {
        if (hasExistingImages) return true
        ...
        <Box>
          ...
          {selectedPackages.length > 0 && (
            <Flex align="center" gap={2} mb={4} px={3} py={2}
              ...
              <Button size="xs" variant="ghost" color="pink.400" ml="auto"
                fontSize="xs" h="auto" p={1} _hover={{ color: "pink.600" }}
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (values.included)
          fd.append("included", JSON.stringify(formatToArray(values.included)))
        if (values.not_included)
    ...
    const fetchOffer = async () => {
      try {
        ...
        formik.setValues({
          ...
          included: arrayToText(o.included),
          not_included: arrayToText(o.not_included),
        })

        // Normalize packages — API returns `destination`, UI expects `destinations`
        const pkgs = (o.packages || []).map((p) => ({
          ...p,
        ...
        toaster.create({ description: "Impossible de charger l'offre.", type: "error", closable: true })
      } finally {
        setLoading(false)
      }
    }
      ...
      <form onSubmit={formik.handleSubmit}>
        <VStack gap={4} align="stretch">
          ...
          <Card title="Informations de l'offre" icon={LuPackage} iconColor="blue">
            <VStack gap={4} align="stretch">
              ...
              <FormField formik={formik} name="title" label="Titre de l'offre"
                ...
                <Input outline="none" {...inp(formik, "title")}
                  placeholder="Ex: Voyage top Vente Istanbul 7 jours" />
              </FormField>

              <Grid templateColumns={{ base: "1fr", sm: "1fr 1fr" }} gap={4}>
                <FormField formik={formik} name="destination" label="Destination"
                  icon={LuMapPin} required>
                  <Input outline="none" {...inp(formik, "destination")}
                    placeholder="Ex: Tunisie, Istanbul" />
                </FormField>
                <FormField formik={formik} name="duration" label="Durée" icon={LuClock}>
                  <Input type="number" outline="none" {...inp(formik, "duration")}
                    placeholder="Ex: 7" />
                </FormField>
              </Grid>
              ...
              <Box>
                ...
                <Flex w="full" align="flex-start"
                  ...
                  <Textarea outline="none" name="description"
                    ...
                    placeholder="Décrivez l'itinéraire, les étapes, les activités…"
                    border="none" bg="transparent" px={0}
          ...
          <Card title="Inclus & non inclus" icon={LuCheck} iconColor="purple">
            <Grid templateColumns={{ base: "1fr", sm: "1fr 1fr" }} gap={4}>
              {[
                { name: "included", label: "Inclus", placeholder: "Hébergement, transport, guide…" },
                { name: "not_included", label: "Non inclus", placeholder: "Vols internationaux, assurance…" },
                ...
                <Box key={name}>
                  ...
                  <Flex w="full" align="flex-start"
                    ...
                    borderRadius="xl" bg="white" px={3} pt={2.5}
                    _focusWithin={{ borderColor: "blue.400", boxShadow: "0 0 0 3px rgba(49,130,206,0.12)" }}>
                    <Textarea outline="none" name={name}
                      value={formik.values[name]}
                      onChange={formik.handleChange}
          ...
          <Box bg="white" borderRadius="2xl" p={6}
            ...
            <Text fontSize="xs" color="gray.400" mb={4}>
              Les photos existantes sont conservées si vous n'en ajoutez pas de nouvelles
            </Text>
            ...
            <FileUpload.Root maxFiles={15} accept="image/*"
              ...
              <FileUpload.Dropzone w="full" border="2px dashed"
                 borderColor={formik.touched.images && formik.errors.images && totalImages === 0
              ? "red.200" : "gray.100"} borderRadius="xl"
                bg="gray.50" py={8} cursor="pointer" transition="all 0.15s"
          ...
          <Flex gap={3} justify="flex-end" pb={4}>
            ...
            <Button type="submit" colorScheme="blue" borderRadius="xl"
              ...
              <Flex align="center" gap={2}>
                <LuGlobe size={14} /> Enregistrer les modifications
              </Flex>
            </Button>
          </Flex>
