...
const EditFlight = () => {
  ...
  useEffect(()=>{
    const flightData = async () => {
      try{
        const res = await AxiosToken.get(`/service/airline/flight/public/get/${id}`)
...
const mapClasses = (flightClasses = []) => {
  const classes = {};
  const classesChildren = {};
  const seatsClasses = {};

  flightClasses.forEach(fc => {
    classes[fc.class_name] = fc.price_adult;
    classesChildren[fc.class_name] = fc.price_children;
    seatsClasses[fc.class_name] = fc.seats_available;
  });

  return { classes, classesChildren, seatsClasses };
};
const mapped = mapClasses(flight?.flightClasses);

  const formik = useFormik({
    enableReinitialize:true,
    initialValues: {
      flight_number: flight?.flight_number || "", from: flight?.departure_airport || "", to: flight?.arrival_airport || "",
      departure: formatDateTimeLocal(flight?.departure) || "", arrival:formatDateTimeLocal(flight?.arrival) || "", duration:flight?.duration || "",
      classes: mapped.classes || {},
    classesChildren: mapped.classesChildren || {},
    seatsClasses: mapped.seatsClasses || {},

      selectedClass: "Économique",
      seats_total: flight?.seats_total || "",
      baggage_kg:flight?.baggage_kg || "23", status:flight?.status || "programmé", type_flight:flight?.type_flight ||"aller retour"
    },
    validationSchema,
    ...
    <Box>
      ...
      <Box mb={8}>
        ...
        <Text fontSize="sm" color="gray.400" mt={1}>
           Modifiez les informations de votre vol
        </Text>
      </Box>
