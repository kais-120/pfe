...
const airportToCity = (code) => {
    ...
    return cities[code?.toLowerCase()] || code?.toUpperCase() || "—"
}
...
function BookingCard({ booking, index }) {
    ...
    const totalAdults = flights.reduce((s, f) => s + (f.adult_passenger_count ?? 0), 0)
    const totalChildren = flights.reduce((s, f) => s + (f.children_passenger_count ?? 0), 0)
                ...
                <Flex align="center" gap={3}>
                    ...
                    <Box>
                        ...
                        <Text fontSize="xs" color="gray.400" mt={0.5}>
                            {booking.userBooking?.email} · {booking.userBooking?.phone}
                        </Text>
                    </Box>
                </Flex>
            ...
            <Box px={6} py={5}>
                ...
                <Grid templateColumns={{ base: "1fr 1fr", sm: "repeat(4, 1fr)" }} gap={3} mb={5}>
                    ...
                    <Box bg="blue.50" borderRadius="xl" p={3}>
                        ...
                        <Text fontSize="sm" fontWeight={700} color="gray.800">
                            {firstFlight?.departure ? formatDate(firstFlight.departure) : "—"}
                        </Text>
                        <Text fontSize="xs" color="gray.400">
                            {airportToCity(firstFlight?.departure_airport)}
                        </Text>
                    </Box>
                    ...
                    <Box bg="blue.50" borderRadius="xl" p={3}>
                        ...
                        <Text fontSize="sm" fontWeight={700} color="gray.800">
                            {firstFlight?.arrival ? formatDate(firstFlight.arrival) : "—"}
                        </Text>
                        <Text fontSize="xs" color="gray.400">
                            {airportToCity(firstFlight?.arrival_airport)}
                        </Text>
                    </Box>
                    ...
                    <Box bg="gray.50" borderRadius="xl" p={3}>
                        <Flex align="center" gap={1.5} mb={1}>
                            <LuPlane size={11} color="gray" />
                            <Text fontSize="9px" fontWeight={700} color="gray.500" textTransform="uppercase" letterSpacing="wider">
                                Type
                            </Text>
                        </Flex>
                        <Text fontSize="sm" fontWeight={700} color="gray.800">
                            {firstFlight?.type_flight ?? "—"}
                        </Text>
                    </Box>
                            ...
                            {flights.map((flight, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <Table.Cell px={4} py={3}>
                                            <Flex align="center" gap={2}>
                                                <Box color="blue.400"><LuPlane size={13} /></Box>
                                        ...
                                        <Table.Cell px={4} py={3}>
                                            <Text fontSize="sm" color="gray.700">
                                                {airportToCity(fd?.departure_airport)} → {airportToCity(fd?.arrival_airport)}
                                            </Text>
                                        </Table.Cell>
                                        <Table.Cell px={4} py={3} isNumeric>
                                            <Flex align="center" justify="flex-end" gap={1}>
                                                <LuUsers size={12} color="gray" />
                                                <Text fontSize="sm" color="gray.700">{flight.adult_passenger_count ?? 0}</Text>
                                            </Flex>
                                        </Table.Cell>
                                        <Table.Cell px={4} py={3} isNumeric>
                                            <Flex align="center" justify="flex-end" gap={1}>
                                                <LuBaby size={12} color="gray" />
                                                <Text fontSize="sm" color="gray.700">{flight.children_passenger_count ?? 0}</Text>
                                            </Flex>
                                        </Table.Cell>
                                        <Table.Cell px={4} py={3}>
                                            <Badge colorScheme="blue" borderRadius="full" fontSize="xs" textTransform="capitalize">
                                                {flight.seat_class ?? "—"}
                                            </Badge>
                                        </Table.Cell>
                                    </Table.Row>
...
export default function BookingAirline() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true)
                const response = await AxiosToken.get("/booking/get/partner/airline")
    ...
    const totalPassengers = bookings.reduce((s, b) =>
        s + (b.flightBooking?.reduce((ss, f) => ss + (f.adult_passenger_count ?? 0) + (f.children_passenger_count ?? 0), 0) ?? 0), 0)
        ...
        <Container py={8}>
            ...
            {!loading && bookings.length > 0 && (
                <Grid templateColumns={{ base: "1fr 1fr", md: "repeat(3, 1fr)" }} gap={3} mb={7}>
                    ...
                    <StatCard icon={LuUsers} label="Total passagers" value={totalPassengers} color="purple" />
                </Grid>
            )}
            ...
            {!loading && !error && bookings.length === 0 && (
                ...
                >
                    ...
                    <Text fontSize="sm" color="gray.400">
                        Les réservations de vols de vos clients apparaîtront ici.
                    </Text>
                </Flex>
            )}
